...
/-- If {a,b} = {a,c}, then b = c.
  ...
  TODO: Can this be replaced by Sym2.congr_right or, at least, proved using it?
-/
...
#eval (({1, 3} : (Finset (Fin 4))).erase 2)

#eval (univ : Finset (Fin 3)) \ {1, 2}

#check α

def PerfectMatching.remove (M : PerfectMatching α) (ij : M.edges) : PerfectMatching ((Finset.univ : Finset α)\ (set ij) : (Finset α)) where
  edges := M.edges.erase ij
  ordered := by
    intro b hb; rw [mem_erase] at hb;
    apply M.ordered; exact hb.2
  disjoint := by
    intro b1 hb1 b2 hb2 hne; rw [mem_erase] at hb1 hb2;
    apply M.disjoint; exact hb1.2; exact hb2.2
    exact hne
  union := by
    intro x
    rcases M.union x with b
    have : b ≠ ij := by rw [mem_erase] at x

  /- ((univ : Finset α) \ (_root_.set ij)) -/
