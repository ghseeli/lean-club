...
- theorem PerfectMatching.partner_invol (M : PerfectMatching α) : M.partner ∘ M.partner = id
-/
...
theorem PerfectMatching.partner_invol (M : PerfectMatching α) : M.partner ∘ M.partner = id := by
  ...
  have h_block : set (M.block i) = {i, M.partner i} := by
    -- By definition of partner, we know that the block containing i is {i, partner i}. Therefore, the set of the block is {i, partner i}.
