...
example : IsZeckendorfRep ([5, 2] : List ℕ) := by
  simp [IsZeckendorfRep]

example : ¬ IsZeckendorfRep ([5, 4] : List ℕ) := by
  simp [IsZeckendorfRep]

@[simp]
lemma IsZeckendorfRep_nil : IsZeckendorfRep [] := by simp [IsZeckendorfRep]

#eval head [2, 1, 5] (by decide)
#eval head? [2, 1, 5]
#eval head? ([] : List ℕ)
...
lemma IsZeckendorfRep.sum_fib_lt : ∀ {n l}, IsZeckendorfRep l → (∀ a ∈ (l ++ [0]).head?, a < n) →
  ...
  | n, a :: l, hl, hn => by
    ...
    have : fib a + (map fib l).sum < fib a + fib (a - 1) :=
      add_lt_add_left (sum_fib_lt hl.2 this) (fib a)
    apply this.trans_le
